...
//function callap left menu
function showItemLeftMenu() {
    var Accordion = function (el, multiple) {
        this.el = el || {};
        this.multiple = multiple || true;
        // Variables privadas
        var links = this.el.find('.ftms-nav-link');
        // Evento
    ...
    Accordion.prototype.dropdown = function (e) {
        ...
        //neeus item cos submenu thi voi toggleClass ftms-open
        $this.parent()[0].childNodes.forEach(el => {
            if (el.classList?.contains("ftms-submenu")) {
                $next.slideToggle();
                $this.parent().toggleClass('ftms-open');
            }
        });

        // $next.toggleClass('ftms-display-block');

        // if (!e.data.multiple) {
        //   $el.find('.ftms-submenu').not($next).slideUp().parent().removeClass('ftms-open');
        // };
    }

    var accordion = new Accordion($('#ftms-accordion'), false);
}
